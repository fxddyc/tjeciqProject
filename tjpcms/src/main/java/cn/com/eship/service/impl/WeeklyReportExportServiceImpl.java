...
public class WeeklyReportExportServiceImpl implements WeeklyReportExportService {
    ...
    private void buildFirstPart() throws Exception {
        ...
        String totalIncrease = String.format("%.0f", (double)(totalCases-lastWeekTotalCases)/(lastWeekTotalCases==0?1:lastWeekTotalCases)*100)+"%";
        ...
        if (tcl != null && tcl.size() > 0) {
            for (Map<String,Object> map:tcl){
                ...
                String reason = "("+ (map.get("reason")!=null?(String)map.get("reason"):"Unexpected change or increase")+")";
                String reasonCn = transformReason(reason);
                if (stringBuilder.length()<1){
                    if(tcl.size()>1) stringBuilder.append("其中，");
                }else {
                    stringBuilder.append("；");
                }
                stringBuilder.append(reasonCn).append(reason).append("的疫情 ");
                stringBuilder.append(cases).append(" 例，占本周疫情发生数量的 ");
                stringBuilder.append(String.format("%.0f", (double)cases/totalCases*100)).append("% ");
            }
        ...
        if (rm.size()>0) textMap.putAll(rm);
    }
    ...
    private void addDiseaseClassData()throws Exception{
        ...
        List<Map<String, Object>> lastWeekCasesList = weeklyReportExportDao.findTotalCasesOfDiseaseClass(lastWeekStartDate,lastWeekEndDate);
        ...
        List<Map<String, Object>> thisWeekCasesList = weeklyReportExportDao.findTotalCasesOfDiseaseClass(this.startDate,this.endDate);
        if (thisWeekCasesList != null && thisWeekCasesList.size() > 0) {
            ...
            textMap.put("${aClassIncrease}",String.format("%.0f", (double)(thisWeekAClassCases-lastWeekAClassCases)/(lastWeekAClassCases==0?1:lastWeekAClassCases)*100)+"%");
            textMap.put("${bClassCases}",thisWeekBClassCases.toString());
            textMap.put("${bClassIncrease}",String.format("%.0f", (double)(thisWeekBClassCases-lastWeekBClassCases)/(lastWeekBClassCases==0?1:lastWeekBClassCases)*100)+"%");
            textMap.put("${cClassCases}",thisWeekCClassCases.toString());
            textMap.put("${cClassIncrease}",String.format("%.0f", (double)(thisWeekCClassCases-lastWeekCClassCases)/(lastWeekCClassCases==0?1:lastWeekCClassCases)*100)+"%");
        }
    }
    ...
    private void buildSecondPart()throws Exception{
        ...
        if (casesList != null && casesList.size() > 0) {
            ...
            for (Map<String, Object> map : casesList){
                ...
                String region = (map.get("regionNameCn")!= null && !"".equals(map.get("regionNameCn")))?(String)map.get("regionNameCn"):(String)map.get("country");
                stringBuilder.append(eName).append("在").append(region).append("共发生 ").append(outbreaks).append(" 例，发布原因: ").append(reasonCn).append(reason);
                switch (diseaseClass) {
                    case "一类":
    ...
    private void buildLastPart()throws Exception{
        List<Map<String, Object>> tcl = weeklyReportExportDao.findDiseaseManifestationData(this.startDate,this.endDate);
        if (tcl != null && tcl.size() > 0) {
            Map<String,StringBuilder> reasonMap = new HashMap<>();
            StringBuilder rs = new StringBuilder("本周疫情中，");
            for (Map<String, Object> map : tcl){
                String reason = map.get("reason")!=null?(String)map.get("reason"):"Unexpected change or increase";
                String manifestation = (map.get("manifestation")!=null&&!"".equals(map.get("manifestation")))?(String)map.get("manifestation"):"不详";
                String reasonCn = transformReason(reason);
                String eName = (map.get("epidemicNameCn") != null && !"".equals(map.get("epidemicNameCn"))) ? (String) map.get("epidemicNameCn") : (String) map.get("disease");
            ...
            if (rs.length()>6) textMap.put("${reasonManifestation}",rs.toString());
        }
    ...
    private void buildThirdPart()throws Exception{
        ...
        List<Map<String, Object>> tcl = weeklyReportExportDao.findTotalCasesGroupByDiseaseClass(this.startDate,this.endDate);
        if (tcl != null && tcl.size() > 0) {
            ...
            for (Map<String,Object> map:tcl){
                ...
                switch (diseaseClass) {
                    case "一类":
                        aClassStr.append(cases).append(" 例，占比").append(String.format("%.0f", (double)cases/totalCases*100)).append("%，涉及 ");
                        aClassStr.append(countryNum).append(" 个国家；");
                        break;
                    case "二类":
                        bClassStr.append(cases).append(" 例，占比").append(String.format("%.0f", (double)cases/totalCases*100)).append("%，涉及 ");
                        bClassStr.append(countryNum).append(" 个国家；");
                        break;
                    case "其他类":
                        cClassStr.append(cases).append(" 例，占比").append(String.format("%.0f", (double)cases/totalCases*100)).append("%，涉及 ");
                        cClassStr.append(countryNum).append(" 个国家；");
                        break;
                }
            }
            if (aClassStr.length()>7) s.append(aClassStr);
            if (bClassStr.length()>7) s.append(bClassStr);
            if (cClassStr.length()>8) s.append(cClassStr);
            if(textMap.size()>0 && s.length()>2) textMap.put("${classification}",s.toString());
        }
    }
    ...
    public String findDiseaseClassData(String startDateCondition)throws Exception{
        if (StringUtils.isNotEmpty(startDateCondition)){
            Map<String,String> dateMap = TimeUtils.getFirstAndLastDayOfThisWeek(new SimpleDateFormat("MM/dd/yyyy").parse(startDateCondition));
            this.startDate = dateMap.get("beginDate");
            this.endDate = dateMap.get("endDate");
        }
        List<Map<String, Object>> casesList = weeklyReportExportDao.findTotalCasesGroupByDiseaseClass(this.startDate,this.endDate);
    ...
    private List<String[]> findDiseaseHistoryDataList() throws Exception{
        ...
        if (casesList != null && casesList.size() > 0) {
            for (Map<String, Object> map : casesList){
                String eName = (map.get("disease")!=null&&!"".equals(map.get("disease")))?(String) map.get("disease"):"";
                String country = (map.get("country")!=null&&!"".equals(map.get("country")))?(String) map.get("country"):"";
                if (condition.length()>1){
                    condition.append(" OR ");
                }
                condition.append("(e.disease='").append(eName.replace("'","\\'")).append("' AND e.country='").append(country.replace("'","\\'")).append("') ");
            }
        }else {
            return null;
        }
    ...
    private List<String[]> findContinentDiseaseHistoryData()throws Exception{
        List<Map<String, Object>> tcl = weeklyReportExportDao.findContinentDiseaseHistoryData(this.startDate,this.endDate);
        List<String[]> tableList = new ArrayList<>();
        if (tcl != null && tcl.size() > 0) {
            for (Map<String,Object> map:tcl){
                String eName = (map.get("epidemicNameCn") != null && !"".equals(map.get("epidemicNameCn"))) ? (String) map.get("epidemicNameCn") : (String) map.get("disease");
    ...
    private List<String[]> findDetailTableData()throws Exception{
        ...
        if (tcl != null && tcl.size() > 0) {
            for (Map<String,Object> map:tcl){
                ...
                String region = (map.get("regionNameCn")!= null && !"".equals(map.get("regionNameCn")))?(String)map.get("regionNameCn"):(String)map.get("country");
                String dateRes = (map.get("dateRes")!=null&& !"".equals(map.get("dateRes")))?(String)map.get("dateRes"):"";
                String reason = map.get("reason")!=null?(String)map.get("reason"):"Unexpected change or increase";
                String manifestation = (map.get("manifestation")!=null&&!"".equals(map.get("manifestation")))?(String)map.get("manifestation"):"不详";
                String reasonCn = transformReason(reason);
                tableList.add(new String[]{date,eName,diseaseClass,reasonCn,manifestation,Integer.toString(outbreaks),dateRes,region,continent,species});
            }
        }
        return tableList;
    }
    ...
    @Override
    public byte[] buildWordDoc(String startDateCondition, String svgDiseaseClassPie, String svgDiseaseDetailBar, String svgDiseaseHistoryBar, String svgReportReasonPie) throws Exception {
